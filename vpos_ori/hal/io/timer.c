...
void vh_timer_irq_enable()
{
	...
	GICD_ICACTIVER((unsigned int)(n/32)) = 1 << (n % 32);
	GICD_ICPENDR((unsigned int)(n/32)) = 1 << (n % 32);

	// enable interrupt
	GICD_ISENABLER((unsigned int)(n/32)) = 1 << (n % 32);

	// set interrupt target (to cpu 0)
	GICD_ITARGETSR((unsigned int)(n/4)) = 1 << ((n % 4) * 8);
}
...
void vk_timer_irq_enable()
{
	...
	write_cntp_tval((unsigned long)(sec_1/16));
	write_cntp_ctl(1);
}
